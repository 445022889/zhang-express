...
app.post("/api/add_company", async (req, res) => {
    if (req.headers["x-wx-source"]) {
        ...
        data['expire-date'] = (parseInt(data.long) || 1) * 86400 + new Date().getTime()
        data["company-salesman"] = 1
        data["company-construction-workers"] = 1
        data["company-supervisors"] = 1
        const duplicates = await mysql.select("count(1)").from("company").where("company-name", data["company-name"]).queryValue()
        const must_in_arr = ["company-name", "company-area", "company-address", "company-master-name", "company-master-mobile", "company-main-brand", "company-type", "company-salesman", "company-construction-workers", "company-supervisors", "expire-date"]
